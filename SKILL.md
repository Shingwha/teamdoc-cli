---
name: teamdoc
description: TeamDoc 知识库命令行工具(td)。安装或升级 td、用命令行操作知识库(令牌登录、项目列表、文档创建/读取/编辑/删除/搜索、云空间文件上传下载、最近动态、API 透传)时使用。涉及 td 命令、TeamDoc CLI、知识库终端操作时触发。
---
...
1. 浏览器打开 TeamDoc → 右上角「个人设置」→「访问令牌」→ 新建,作用域选 **read,write**(只读令牌无法执行任何写命令),令牌明文只显示一次。
...
td project ls [--all]                       # 项目列表(管理员 --all 看全部)
td doc ls <项目ID>                          # 文档树
...
用 `td file up --json` 拿文件 ID;`td doc search` 查文档 ID。外部 http(s) 链接保持原样即可。
...
4. **建文档**:`td doc new <项目> <标题> -`,改写后的正文从 stdin 灌入;追加到既有文档用 `td doc edit <id> -`。
